...
def test_get_or_create_player_is_case_insensitive_and_idempotent(pg_url):
    conn = db.get_connection(pg_url)
    db.init_db(conn)
...
def test_upsert_pick_overwrites_existing_pick_for_same_game(pg_url):
    conn = db.get_connection(pg_url)
    db.init_db(conn)
...
def test_get_player_picks_for_games_returns_empty_dict_for_no_games(pg_url):
    conn = db.get_connection(pg_url)
    db.init_db(conn)
    player = db.get_or_create_player(conn, "Chris", _now())
    conn.commit()
...
def test_get_decided_picks_only_returns_final_games(pg_url):
    conn = db.get_connection(pg_url)
    db.init_db(conn)
...
def test_join_get_renders_name_form(pg_url, monkeypatch):
...
def test_join_post_creates_player_and_sets_cookie(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
...
def test_join_post_reuses_existing_player_case_insensitively(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
...
def test_join_post_rejects_empty_name(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
...
def test_base_header_shows_picking_as_name_after_join(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    client.post("/join", data={"name": "Chris", "next": "/"})
    response = client.get("/")
...
def test_submit_pick_without_player_redirects_to_join(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
...
def test_submit_pick_rejected_after_kickoff(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    client.post("/join", data={"name": "Chris", "next": "/"})
    ...
    check_conn = db.get_connection(pg_url)
    row = check_conn.execute("SELECT * FROM picks WHERE game_id = 'g_started'").fetchone()
    check_conn.close()
    assert row is None


def test_pickem_page_shows_standings_and_weekly_breakdown(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    _push_g1_kickoff_into_future(pg_url)
    client.post("/join", data={"name": "Chris", "next": "/"})
...
def test_pickem_page_treats_tie_game_as_push(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    _push_g1_kickoff_into_future(pg_url)
    client.post("/join", data={"name": "Chris", "next": "/"})
    client.post("/games/g1/pick", data={"team_id": "A", "week": 1})
...
def test_pickem_page_shows_joined_player_with_no_decided_picks(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    client.post("/join", data={"name": "Chris", "next": "/"})
...
def test_submit_pick_rejects_team_not_in_game(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    _push_g1_kickoff_into_future(pg_url)
    client.post("/join", data={"name": "Chris", "next": "/"})
...
def test_pickem_page_does_not_crash_on_final_game_with_null_scores(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    _push_g1_kickoff_into_future(pg_url)
    client.post("/join", data={"name": "Chris", "next": "/"})
    client.post("/games/g1/pick", data={"team_id": "A", "week": 1})
...
def test_schedule_page_shows_pick_mode_toggle_and_pick_buttons(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    _push_g1_kickoff_into_future(pg_url)
    client.post("/join", data={"name": "Chris", "next": "/"})
...
def test_submit_pick_from_index_redirects_back_to_index(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    _push_g1_kickoff_into_future(pg_url)
    client.post("/join", data={"name": "Chris", "next": "/"})
...
def test_submit_pick_from_index_persists_and_highlights_active_pick(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    _push_g1_kickoff_into_future(pg_url)
    client.post("/join", data={"name": "Chris", "next": "/"})
...
def test_submit_pick_from_index_rejected_after_kickoff_redirects_with_error(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    client.post("/join", data={"name": "Chris", "next": "/"})
...
def test_index_shows_correct_pick_badge_for_final_game(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    _push_g1_kickoff_into_future(pg_url)
    client.post("/join", data={"name": "Chris", "next": "/"})
    client.post("/games/g1/pick", data={"team_id": "A", "week": 1})
...
def test_index_shows_push_badge_for_tied_final_game(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
    _push_g1_kickoff_into_future(pg_url)
    client.post("/join", data={"name": "Chris", "next": "/"})
    client.post("/games/g1/pick", data={"team_id": "A", "week": 1})
...
def test_current_player_ignores_tampered_cookie_instead_of_crashing(pg_url, monkeypatch):
    client = make_test_client(pg_url, monkeypatch)
